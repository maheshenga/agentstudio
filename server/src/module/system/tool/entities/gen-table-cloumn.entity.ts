...
@Entity('sa_tool_generate_columns', {
  comment: '代码生成业务表字段',
})
export class GenTableColumnEntity extends BaseEntity {
  ...
  @Column({ type: 'tinyint', name: 'is_pk', default: 0, comment: '是否主键（1是）' })
  public isPk: number;

  @ApiProperty({ type: String, description: '是否自增（1是）' })
  @Column({ type: 'tinyint', name: 'is_increment', default: 0, comment: '是否自增（1是）' })
  public isIncrement: number;

  @ApiProperty({ type: String, description: '是否必填（1是）' })
  @Column({ type: 'tinyint', name: 'is_required', default: 0, comment: '是否必填（1是）' })
  public isRequired: number;

  @ApiProperty({ type: String, description: '是否为插入字段（1是）' })
  @Column({ type: 'tinyint', name: 'is_insert', default: 0, comment: '是否为插入字段（1是）' })
  public isInsert: number;

  @ApiProperty({ type: String, description: '是否编辑字段（1是）' })
  @Column({ type: 'tinyint', name: 'is_edit', nullable: true, default: 0, comment: '是否编辑字段（1是）' })
  public isEdit: number;

  @ApiProperty({ type: String, description: '是否列表字段（1是）' })
  @Column({ type: 'tinyint', name: 'is_list', nullable: true, default: 0, comment: '是否列表字段（1是）' })
  public isList: number;

  @ApiProperty({ type: String, description: '是否查询字段（1是）' })
  @Column({ type: 'tinyint', name: 'is_query', nullable: true, default: 0, comment: '是否查询字段（1是）' })
  ...
  @Column({ type: 'varchar', name: 'column_default', length: 200, default: null, nullable: true, comment: '默认值' })
