...
describe('AppRuntimeController', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    controller = new AppRuntimeController(sessionService as any, contextService as any);
  });
  ...
  it('is public to the JWT guard but authorizes the dedicated runtime token', async () => {
    ...
    const request = {
      headers: { 'x-app-runtime-token': 'runtime-token', 'x-request-id': 'req-1', 'user-agent': 'jest' },
