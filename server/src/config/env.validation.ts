...
export const envValidationSchema = Joi.object({
  ...
  APP_PACKAGE_MAX_FILE_MB: Joi.number().integer().min(1).max(100).default(25),
  APP_PACKAGE_MAX_UNCOMPRESSED_MB: Joi.number().integer().min(1).max(2048).default(200),
  APP_PACKAGE_MAX_COMPRESSION_RATIO: Joi.number().integer().min(1).max(1000).default(100),
